...
/// @title NewsSubscriptionV2 — TEST-ONLY upgrade fixture (not a product version).
...
///         under `test/` on purpose. When a real upgrade is needed, write the actual V2 in `src/`
///         (plus a `script/Upgrade.s.sol`) and validate against `NewsSubscription` the same way.
